...
const ProductList = () => {
  ...
  const handleEditClick = (product) => {
    ...
    product.Sizes.forEach((size) => {
      stockObject[size.name] = size.Stock.quantity;
    });
    setEditedStock({ ...stockObject });
  };
                ...
                {editingProduct === product ? (
                  <div>
                    {product.Sizes.map((size) => (
                      <div key={size.Id}>
                        <label htmlFor={`size_${size.Id}`}>{size.name}</label>
                        <input
                          type="number"
                          value={editedStock[size.name] || 0}
                          onChange={(e) =>
